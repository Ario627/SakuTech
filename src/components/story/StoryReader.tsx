...
import QuizTutorial, { hasSeenTutorial } from "../quiz/QuizTutorial";
...
export default function StoryReader({ chapter, onNextChapter }: StoryReaderProps) {
  ...
  return (
    <>
      {showTutorial && <QuizTutorial />}

      <div
        className="min-h-dvh bg-warm-bg"
        style={{
        ...
        <main className="pt-6 pb-32">
          ...
          {showContinue && (
            <motion.div
              ...
              className="flex justify-center mt-10 px-5"
            >
              <button
                onClick={onNextChapter}
                className="w-full max-w-70 py-3.5 px-6 rounded-xl bg-rose-accent text-white font-semibold text-[0.9375rem] active:scale-[0.97] transition-transform font-ui"
              >
                Lanjutkan &#9654;
              </button>
            </motion.div>
